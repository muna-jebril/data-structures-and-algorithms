...
Write a function named count that, given an integer
 and an array of arrays, uses either filter, map, or reduce to count
    ...
    return input.map(element => {
        return element.filter(value => {
          if (typeof (value) === 'number' && value % 5 === 0)
           return value;
        }).map(value =>{ return Math.pow(2, value)});
      });
...
let findMaleAndFemale = (data) => {
  return data.filter(element =>
    {if(element.gender === 'male' || element.gender === 'female') return element;})
